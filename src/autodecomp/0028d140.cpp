// coverage: addr=0x0028d140 symbol=_ZN2Tz8ItemStatC1Ev size=72 class=straight source=typed status=exact fuzzy=100.0
// coverage-alternatives: lift:exact:100.0;m2c:exact:100.0;ghidra:near:66.45
#include "common/types.h"
typedef float f32; typedef double f64;
struct Tz_ItemStat_Info;
struct YS_ITEM_PARAM;
struct Tz_ItemStat;
...
struct Tz_ItemStat_Info {  // structure Tz::ItemStat::Info size=0x2bd8
    dk_Sprite *Parent;                                 // +0x0
    u8 Mess[2220];   // raw: dk::SpriteMessage[5] (size mismatch) // +0x4
    u8 Mess2[2220];   // raw: dk::SpriteMessage[5] (size mismatch) // +0x8b0
    u8 Num[6660];   // raw: dk::SpriteMessage[15] (size mismatch) // +0x115c
    s8 NumBuff[120];                                   // +0x2b60
};

struct YS_ITEM_PARAM {  // class YS::ITEM_PARAM size=0x10
    u16 Id;                                            // +0x0
    u16 AbilityItem;                                   // +0x2
    u8 Attack;                                         // +0x4
    u8 Wisdom;                                         // +0x5
...
struct Tz_ItemStat {  // class Tz::ItemStat size=0x2c14
    Tz_ItemStat_Info m_Info;                           // +0x0
    Tz_Layout *m_Lay;                                  // +0x2bd8
    s32 m_SeqUnit;                                     // +0x2bdc
    s32 m_ImgUnit;                                     // +0x2be0
    s32 m_Pri;                                         // +0x2be4
    s16 m_FontColorSeqNum[7];                          // +0x2be8
    s16 m_SeqNum[7];                                   // +0x2bf6
    YS_ITEM_PARAM m_DummyParam;                        // +0x2c04
};
...
u32 lift_0028d140(u32 a0) {
    *(u32*)&(((Tz_ItemStat*)a0)->m_Info.Parent) = (u32)&D_00347e60;
    *(u32*)&(((Tz_ItemStat*)a0)->m_Info.Mess[8]) = 0;
    func_00239700(a0 + 0x18);
    *(u32*)&(((Tz_ItemStat*)a0)->m_Info.Parent) = (u32)&D_0035f420;
    return func_0028d188(a0);
}
