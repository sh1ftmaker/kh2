// coverage: addr=0x00253948 symbol=func_00253948 size=44 class=branchy source=ghidra status=exact fuzzy=100.0
// coverage-alternatives: lift:near:41.07;m2c:near:41.07
...
s32 FUN_00253948(...) asm("func_00253948");

// Ghidra decompile of 0x00253948, wrapped mechanically (ad/fallback.py)

uint FUN_00253948_impl(uint param_1) asm("func_00253948");
uint FUN_00253948_impl(uint param_1) {
  if ((param_1 & 0x3f) != 0) {
    if ((int)param_1 < 0) {
      param_1 = param_1 + 0x3f;
    }
    param_1 = ((int)param_1 >> 6) * 0x40 + 0x40;
  }
  return param_1;
}
