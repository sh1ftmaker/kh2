...
# Task D: a per-EE-address prototype table generated by gen_prototypes.py from
# the autodecomp E3(PS3) DWARF (map_functions.tsv + funcs_dwarf.json).  Far
# more trustworthy than E3_MAP above: it knows the true explicit param list
# *and* whether a member function is static (E3_MAP's demangled text can
# never distinguish "static member" from "non-static member with `this`
# omitted", since Itanium mangling never encodes `this` either way).  See
# tools/rig/gen_prototypes.py for the full rationale.
PROTOTYPES_TSV = ROOT / "data" / "prototypes.tsv"
...
_prototypes: Optional[Dict[int, dict]] = None


def prototypes_by_addr() -> Dict[int, dict]:
    """ee_addr -> DWARF-verified prototype row (see gen_prototypes.py)."""
    global _prototypes
    if _prototypes is not None:
        return _prototypes
    out: Dict[int, dict] = {}
    if PROTOTYPES_TSV.exists():
        lines = PROTOTYPES_TSV.read_text(errors="ignore").splitlines()
        if lines:
            hdr = lines[0].split("\t")
            for line in lines[1:]:
                if not line.strip():
...
def e3_map() -> Dict[int, dict]:
    """addr -> {mangled, demangled, decl_file, confidence, ...}.

    Task D: this used to be a straight parse of E3_MAP (ee_symbols_mapped.tsv)
    -- an address-keyed guess table that get_context.py trusts verbatim for
    both the callee extern-decl argument list (callee_decls()) and the
    target's own member/static shape (build_skeleton()).  Two problems with
    trusting it verbatim:

      1. It can be STALE: the registry (functions.tsv) may since have been
         given a different, better name for the same address (see
         docs/e3/additions_conflicts.tsv) -- e.g. 0x0028b040 is registered as
         `Tz::JmReport::GetData(unsigned int)` (1 explicit arg) but this map
         still calls it `Tz::Select::GetSelectPos()` (0 args); get_context.py
         built its extern "C" callee declaration with 0 args, silently
         dropping a real argument register.
      2. It never distinguishes a static member function from a non-static
         one: Itanium mangling has no notation for an implicit `this`, so
         both look identical in a plain demangled string.

    Both are fixed here, without touching get_context.py, by overlaying two
    corrections onto the raw rows before returning them:

      * prototypes_by_addr() (real E3 DWARF, knows `this`/static) wins when
        present for an address -- its ready-built "text" already has `this`
        spliced in as an explicit arg for non-static members, or a
        "[DWARF: static -- ...]" note for static ones.
      * failing that, if the registry's own mangled symbol for this address
        disagrees with this table's "mangled" column (a recorded naming
        conflict), prefer the registry's symbol -- it is what will actually
        be asm()-linked, so its own (correctly Itanium-mangled, hence
        correctly-arity) name is authoritative over a stale guess.
    """
    global _e3_map
    ...
    protos = prototypes_by_addr()
    reg = registry_symbols()
    for addr, row in out.items():
        p = protos.get(addr)
        rsym = reg.get(addr)
        # A naming conflict: the registry has already assigned a *different*
        # mangled symbol to this address than the one E3 knows it by (see
        # docs/e3/additions_conflicts.tsv).  get_context.py derives a
        # target's own skeleton signature by splicing together the class/
        # method NAME from this dict's "demangled" with the ARGUMENT LIST
        # from prototype_for(<the registry symbol>) -- two independent
        # lookups.  If we hand back the E3/DWARF name here while the
        # registry symbol still resolves its own (different) argument list,
        # those two halves silently Frankenstein together into a signature
        # that matches neither name (observed: 0x0028b040 registry symbol
        # `_ZN2Tz8JmReport7GetDataEj` vs. DWARF `_ZN2Tz6Select12GetSelectPosEv`
        # produced the spurious hybrid `_ZN2Tz6Select12GetSelectPosEj`).  So
        # when there IS a conflict, prefer the registry's own name for BOTH
        # halves -- it is self-consistent by construction, since it is the
        # one and only source get_context.py's argument-list half ever
        # reads from.  The richer DWARF text (this-pointer spliced in,
        # static annotated) is used only when there is no such conflict.
        conflict = bool(rsym and rsym.startswith("_Z") and row.get("mangled") and rsym != row["mangled"])
        if p and not conflict:
            row["demangled"] = p["text"]
            if p.get("dwarf_linkage"):
                row["mangled"] = p["dwarf_linkage"]
            if p.get("decl_file"):
                row["decl_file"] = p["decl_file"]
            row["confidence"] = f"dwarf ({p.get('tier', '')})"
            continue
        if conflict:
            dem = cxxfilt(rsym)
            if dem:
                row["demangled"] = dem
                row["mangled"] = rsym
                row["confidence"] = (row.get("confidence") or "") + " (registry name preferred over stale/conflicting E3 map guess)"
    _e3_map = out
    return out
...
def dwarf_by_linkage() -> Dict[str, dict]:
    ...
    # Task D: also mark "source: dwarf" + decl_file for every address the new,
    # better DWARF table (gen_prototypes.py) covers, keyed by whatever symbol
    # will actually be asm()-linked at that address (the registry's, when
    # registered) so get_context.py's prototype_for(symbol) -- which looks
    # this dict up by the exact symbol it was handed -- finds it.
    reg = registry_symbols()
    for addr, p in prototypes_by_addr().items():
        for key in (reg.get(addr), p.get("dwarf_linkage")):
            if key and key not in out:
                out[key] = {
                    "linkage_name": key,
                    "qualified": p.get("text", ""),
                    "decl_file": p.get("decl_file", ""),
                    "low_pc": f"0x{addr:08x}",
                }
    _dwarf = out
    return out
